...
```{r}
dif_mammels <- setdiff(rownames(df_mammals), rownames(df_animals))
dif_mammels
len_m = length(dif_mammels)

dif_animals <- setdiff(rownames(df_animals), rownames(df_mammals))
dif_animals
len_a = length(dif_animals)

total_dif <- len_m + len_a
total_dif

#Como podemos observar hay 11 animales que no están en ambos conjuntos, 3 de ellos son dinosaurios que se encuentran en animals. Los 8 restantes son animales que se encuentran en ambos conjuntos pero con nombres ligeramente distintos.
```
...
#3 de los animales son dinosaurios que se encuentran en el conjunto animals. Los 8 restantes son animales que se encuentran en ambos conjuntos pero con nombres ligeramente distintos.
...
#Como podemos comprovar presentan un comportamiento similar, pero no idéntico. En cuanto a la distribución Gaussiana, podemos afirmar que ambas series de puntos la siguen dado que todos los puntos se encuentran dentro de las líneas discontínuas.
...
libReturn <- library()
libReturn
  #Al ejecutar la variable se nos abre una nueva pestaña donde se nos muestran todos los paquetes instalados.

#Apartado B: primeros 5 elementos de la matriz.
head(libReturn$results, n=5)

#Apartado C:
length(libReturn$results) / 3
  #Tengo instaladas 223 librerias.
...
#Como podemos observar en la gráfica, ambos gráficos muestran todos los puntos dentro de de las líneas discontínuas, como hemos visto anteriormente esto indica que ambas distribuciones son Gaussianas.
...
```{r}
ggplot(cabbages, aes(VitC,fill = Cult)) + geom_histogram(alpha=0.5, col = "black")
#Como se observa en el histograma, si que existen diferencias entre los contienidos de vitamina C para las diferentes variedades de calabazas.
...
```{r}
#Apartado A
plot(cabbages$VitC ~ cabbages$HeadWt) #Este gráfico compara la vitamina c con respecto al peso de la calabaza.
#Como se observar no existe relación lineal.
